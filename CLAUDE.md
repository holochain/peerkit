...
- `packages/peerkit` — `@peerkit/peerkit`, the orchestrator. Owns `AgentKeyPair` (the concrete Ed25519 key implementation), key lifecycle, and `AgentInfo` serialization/deserialization (the wire format for agent-info exchange). Uses `@noble/ed25519` and `@noble/hashes` — chosen for zero dependencies and identical behaviour on Node.js and browsers without polyfills; React Native requires the `react-native-get-random-values` polyfill.
...
- `packages/relay` — `@peerkit/relay`, quick-setup bootstrap/relay node. Wraps `PeerkitRelayBuilder` from `@peerkit/peerkit` with lifecycle wiring (structured logging, OpenTelemetry metrics via `@peerkit/metrics`, an in-memory `@peerkit/agent-store`, signal-driven shutdown). The caller passes a `RelayConfig` (raw `NetworkAccessBytes` + a `networkAccessHandler` — the secret→bytes mapping is the caller's concern, not the package's) and either drives the full process with `run` or embeds a relay with `startRelay`. App-like, not consumed by other packages; excluded from typedoc like `@peerkit/cli`. Library only for now (no CLI/bin or deployment artifacts yet).
...
- **Outbound backpressure.** `Stream.send()` returning `false` means libp2p buffered the frame but must not receive more until `drain`; the yamux write buffer is unbounded by default, so ignoring it grows memory without limit. `MessageStream` (in `transport-libp2p-core/src/message-stream.ts`) owns one wrapper per `/peerkit/message/v1` stream, writes one frame at a time, and waits for `drain`. Do not call `stream.send()` on message streams from anywhere else. `Stream.onDrain()` is not used because `@libp2p/utils` 7.2.1 never resets its promise after the first drain.
